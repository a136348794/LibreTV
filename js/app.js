// 全局变量
let selectedAPIs = JSON.parse(localStorage.getItem('selectedAPIs') || '["tyyszy","dyttzy", "bfzy", "ruyi","非凡","lzi","zy360"]'); // 默认选中资源
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!localStorage.getItem('hasInitializedDefaults')) {
        // 默认选中资源
        selectedAPIs = ["tyyszy","dyttzy", "bfzy", "ruyi","非凡","lzi","zy360"];
...
function resetSearchArea() {
    ...
    try {
        ...
        // 更新页面标题
        document.title = `大猫 - 免费在线视频搜索与观看平台`;
    } catch (e) {
...
async function importConfigFromUrl() {
    ...
    document.getElementById('confirmUrlImport').addEventListener('click', async () => {
        ...
        try {
            ...
            if (config.name !== '大猫-Settings') throw '配置文件格式不正确';
...
async function exportConfig() {
    ...
    config['name'] = '大猫-Settings';  // 配置文件名，用于校验
    ...
    // 将配置数据保存为 JSON 文件
    saveStringAsFile(JSON.stringify(config), '大猫-Settings_' + times + '.json');
}
